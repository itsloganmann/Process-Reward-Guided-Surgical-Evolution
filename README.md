...
Initializes two vLLM pipelines (generator and PRM) and exposes batched generate and score helpers. Models are loaded lazily on first use, allowing the class to be instantiated without a GPU for testing.
...
Thread-safe accounting of prompt tokens, completion tokens, wall-clock GPU latency, and peak VRAM consumed across the entire search. Provides summary dictionaries suitable for logging or Pandas DataFrames.
...
Run the main script to execute PRSE and Best-of-N on five sample GSM8K problems:
...
The script will output a comparative table showing accuracy, token usage, and wall-clock time for both methods.
...
best_trace, tracker = solver.solve("What is 2 + 2?")
...
Every step in every trace is scored by the PRM on a 0.0 to 1.0 scale. The aggregate fitness of a trace is the mean of its step scores. A trace is marked as terminal if its final step contains a boxed answer.
...
| `max_new_tokens` | 1024 | Token budget per generation call |
...
| `max_model_len` | 4096 | Maximum context length for vLLM |
